...
说明: 请求体结构在 SIT 已实测通过(6 个策略全部 code:0), UAT 沿用同一结构。
      UAT 的 TOKEN 当前已失效(110002), 腿代码是否在 UAT 有效需换上有效 TOKEN 后复测,
      如报 400064/400060 则按 UAT 环境的真实期权标的替换 COMBO_STRATEGIES 里的 symbol。
"""
import os
import sys
