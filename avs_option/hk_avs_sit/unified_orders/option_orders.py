...
def account_for(business_type):
    """按业务类型选资金账号: OS(沽空/买入平仓)用沽空账号, O(普通期权)用默认账号。
    普通账号下 OS 单会被 810006 "请先开户" 拦, 所以必须区分。"""
    if business_type == BUSINESS_TYPE_OPTION_SHORT:
        return SHORT_CAPITAL_ACCOUNT
...
def build_option_body(side=SIDE_BUY, business_type=BUSINESS_TYPE_OPTION,
                      ...
                      capital_account=None, **override):
    """
    按 OptionSingleOrderAdminRequest 拼请求体。
    选填字段为 None 时不放进 body, 避免服务端把 null 当非法值。
    capital_account 不传时按 business_type 自动选(见 account_for)。
    """
    body = {
        # 必填: 资金账号。OS 自动用沽空账号, 也可用 capital_account 显式覆盖
        "capitalAccount": capital_account or account_for(business_type),
...
def create_option_short(**kw):
    """
    期权沽空开仓 (businessType=OS, side=2)。
    资金账号由 build_option_body 按 businessType 自动选沽空账号(SHORT_CAPITAL_ACCOUNT),
    普通账号下 OS 单会报 810006 "请先开户"。
...
def create_option_close_short(hold_id=None, **kw):
    """
    买入平仓 (businessType=OS + side=1), 平掉沽空建立的空头持仓。
    不能用普通买入(businessType=O)平空头, 会报 830018
      "您当前持有相同的空头持仓...请先买入平仓空头持仓后再进行普通买入"
    需先有空头持仓, 否则报 800017 "持仓不足"。
    """
    return send_order("HK期权买入平仓(OS)", url_for("option_create"),
                      build_option_body(side=SIDE_BUY, hold_id=hold_id,
                                        business_type=BUSINESS_TYPE_OPTION_SHORT, **kw))
